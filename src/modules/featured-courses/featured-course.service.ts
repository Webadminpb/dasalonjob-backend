...
import {
  CreateFeaturedCourseDto,
  UpdateFeaturedCourseDto,
} from './dto/featured-course.dto';
...
export class FeaturedCourseService {
  ...
  async create(body: CreateFeaturedCourseDto) {
    ...
    if (isExistingCourse.length > 0)
      throw new NotFoundException('Featured Course Already Exists!');
  ...
  async findOne(id: string) {
    ...
    if (!isExistingCourse)
      throw new NotFoundException('Featured Course Not Found!');

    return new ApiSuccessResponse(
      true,
      'Featured course retrieved',
      isExistingCourse,
    );
  }
  ...
  async update(id: string, body: UpdateFeaturedCourseDto) {
    ...
    const updatedFeaturedCourse =
      await this.prismaService.featuredCourse.update({
        where: {
          id,
        },
        data: {
  ...
  async remove(id: string) {
    ...
    return new ApiSuccessResponse(true, 'Featured Course Updated', null);
  }
